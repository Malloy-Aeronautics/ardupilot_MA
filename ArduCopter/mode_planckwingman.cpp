...
bool ModePlanckWingman::init(bool ignore_checks){
    ...
    if(copter.mode_plancktracking.init_without_RTB_request(ignore_checks)) {
      	...
      	copter.planck_interface.request_move_target(Vector3f(0,0,0),true,copter.pos_control->get_max_speed_up_cms(),copter.pos_control->get_max_speed_down_cms());
      	return true;
    }
...
void ModePlanckWingman::run() {
  	...
  	if( !copter.failsafe.radio && copter.flightmode != &copter.mode_auto) {
    	...
    	if(millis() > _next_req_send_t_ms) {
      		...
      		copter.planck_interface.request_move_target(rate_NED, true,copter.pos_control->get_max_speed_up_cms(),copter.pos_control->get_max_speed_down_cms());
